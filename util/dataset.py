...
class CocoDataset(Dataset):
    ...
    def __init__(self, root, ann_path, img_path,
                 way=5, shot=2, query_batch=16, is_cuda=False, catIds=None, init=True):
    ...
    def random_instance(self):
        instances = []
        for cat_id in tqdm(self.coco.cats.keys()):
            anns = self.coco.getAnnIds(catIds=cat_id)
            anns = random.sample(anns, self.shot)
            support, _ = self.crop_support_bg(anns, [cat_id])
            if self.is_cuda:
                instances.append([s.cuda() for s in support])
            else:
                instances.append(support)
