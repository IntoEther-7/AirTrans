...
def test_iteration(dataset, model, save_images):
    ...
    for index, item in enumerate(pbar):
        ...
        # img: PIL.Image.Image = t(img_tensor)
        # att_folder_name = os.path.split(img_info['file_name'])[1].split('.')[0]
        # path = os.path.join('show_attention1', '1', os.path.split(img_info['file_name'])[1])
        # img.save(path)
        # p = os.path.join('show_attention1', att_folder_name)
